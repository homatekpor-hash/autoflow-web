...
export default function ReportsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      af(`/api/reports/revenue?period=${period}`),
      af(`/api/reports/jobs?period=${period}`),
    ]).then(([r,j]) => { setRevenue(r); setJobs(j); setLoading(false); });
  }, [period]);

  function exportCSV() {
    window.open(`${API}/api/reports/export?token=${tok()}`, "_blank");
  }
  ...
  return (
    <div className="fade-up">
      <PageHeader title="Reports & Analytics" subtitle="Revenue, jobs and performance">
        <div className="flex gap-2 items-center">
          ...
          <button onClick={exportCSV} className="rounded-lg border border-ink-ghost px-3 py-1.5 text-xs font-medium text-ink-subtle hover:bg-ink-paper transition">
            📥 Export CSV
          </button>
        </div>
      </PageHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              ...
              ["Completion rate", `${revenue?.completionRate||0}%`,                     "text-orange-500"],
            ].map(([l,v,c])=>(
          ...
          {/* Jobs by status */}
          {jobs?.byStatus && jobs.byStatus.length > 0 && (
            <Card className="p-5">
