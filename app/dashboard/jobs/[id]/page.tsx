...
export default function JobDetailPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    ...
    if (jobRes.ok) { const d = await jobRes.json(); setJob(d); setEst((d.estimate?.items||[]).map((i:any) => ({...i, unitPrice: i.rate || i.unitPrice || 0}))); }
  ...
  async function saveEstimate() {
    ...
    const total = mappedItems.reduce((s,i) => s+(i.quantity*i.rate), 0);
    if (job.estimate?.id) {
      await af(`/api/estimates/${job.estimate.id}`, { method:"PUT", body: JSON.stringify({ items: mappedItems, total }) });
    } else {
      await af(`/api/jobs/${params.id}/estimate`, { method:"POST", body: JSON.stringify({ items: mappedItems, total }) });
    }
  ...
  function printInvoice() {
    ...
    const items = job.estimate?.items?.map((i:any) =>
      `<tr><td>${i.description}</td><td>${i.type}</td><td>${i.quantity}</td><td>GHS ${i.unitPrice}</td><td>GHS ${(i.quantity*i.unitPrice).toFixed(2)}</td></tr>`
    ).join("") || "";
    ...
    <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:30px">
      <div><h1>AutoFlow Ghana</h1><p style="color:#888">Workshop Management Platform</p></div>
    ...
    <div class="meta">
      ...
      <div class="box"><strong>Job reference</strong><br>${job.jobRef}<br>Date: ${new Date(inv.createdAt).toLocaleDateString("en-GH")}</div>
    </div>
    ...
    <div style="text-align:right;margin-top:20px">
      ...
      <p>VAT (12.5%): GHS ${Number(inv.vat).toFixed(2)}</p>
    ...
    <div className="fade-up max-w-4xl">
      ...
      {tab==="notes" && (
        <div className="space-y-4">
          ...
          {notes.length===0 ? (
            <div className="text-center py-10 text-ink-faint text-sm">No notes yet. Be the first to add one.</div>
          ) : (
            notes.map(n=>(
              <div key={n.id} className={clsx("rounded-xl border p-4", n.isInternal?"border-amber-200 bg-amber-50":"border-ink-ghost bg-white")}>
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {n.isInternal && <span className="text-[10px] rounded-full bg-amber-100 text-amber-700 px-2 py-0.5 font-medium">Internal</span>}
                  </div>
          ...
          <div className="rounded-xl border border-ink-ghost bg-white p-5">
            ...
            {estItems.map((item,i)=>(
              <div key={i} className="grid grid-cols-[1fr_80px_80px_100px_32px] gap-2 mb-2 items-center">
                ...
                <input type="number" value={item.unitPrice} onChange={e=>{const a=[...estItems];a[i]={...a[i],unitPrice:+e.target.value};setEst(a);}} className="input text-sm" min="0" step="0.01"/>
                <button onClick={()=>setEst(estItems.filter((_,j)=>j!==i))} className="text-red-400 hover:text-red-600 text-lg">×</button>
              </div>
            ))}
            <button onClick={()=>setEst([...estItems,{description:"",type:"LABOUR",quantity:1,unitPrice:0}])}
              ...
              <div className="border-t border-ink-ghost pt-4 space-y-2 mb-4">
                {job.estimate?.items?.map((item:any,i:number)=>(
                  <div key={i} className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">GHS {(item.quantity*item.unitPrice).toLocaleString()}</span>
                  </div>
                ))}
              </div>
              <div className="border-t border-ink-ghost pt-3 space-y-1">
                <div className="flex justify-between text-sm"><span className="text-ink-faint">Subtotal</span><span>GHS {Number(job.invoice.subtotal).toLocaleString()}</span></div>
                <div className="flex justify-between text-sm"><span className="text-ink-faint">VAT 12.5%</span><span>GHS {Number(job.invoice.vat).toFixed(2)}</span></div>
