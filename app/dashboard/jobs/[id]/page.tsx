          ...
          {!job.invoice ? (
            ...
            <div className="rounded-xl border border-ink-ghost bg-white p-6">
              ...
              <div className="border-t border-ink-ghost pt-4 space-y-2 mb-4">
                {job.estimate?.items?.map((item:any,i:number)=>(
                  <div key={i} className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">GHS {(item.quantity*(item.rate||item.unitPrice||0)).toLocaleString()}</span>
                  </div>
                ))}
              </div>
              <div className="border-t border-ink-ghost pt-3 space-y-1">
                ...
                <div className="flex justify-between text-sm"><span className="text-ink-faint">VAT 12.5%</span><span>GHS {Number(job.invoice.tax).toFixed(2)}</span></div>
